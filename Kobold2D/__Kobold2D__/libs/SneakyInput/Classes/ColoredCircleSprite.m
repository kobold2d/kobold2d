...
-(void) setRadius: (float) size
{
	...
	for(int i=0; i<numberOfSegments; i++)
	{
		...
		float j = radius_ * [[CCDirector sharedDirector] contentScaleFactor] * cosf(theta) + _position.x;
		float k = radius_ * [[CCDirector sharedDirector] contentScaleFactor] * sinf(theta) + _position.y;
...
{
	return [NSString stringWithFormat:@"<%@ = %p | Tag = %i | Color = %02X%02X%02X%02X | Radius = %1.2f>",
			[self class], self, _tag, color_.r, color_.g, color_.b, opacity_, radius_];
}
